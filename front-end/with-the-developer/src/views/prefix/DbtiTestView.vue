...
const questions = ref([
  { question: "동물모양 쿠키 반죽을 만들던 도중, 모양이 이상한 반죽을 발견한다. <br><br>이때, 나의 행동은?",
    answers: ["이건 아트다! 어쩌면 현대미술에 출품할 수 있을지도?",
        "반죽을 다시 동물답게 만든다. <br><br>누가 봐도 고양이여야지!",
      "쿠키는 원래 반듯해야 한다. <br><br>모양틀 어디 갔지?",
      "오... <br><br>이상하지만 구우면 어차피 맛은 똑같으니 괜찮아!"],
    score: [3, 1, 2, 0]
  },

  { question: "친구가 최신 스마트폰을 자랑한다. <br><br>이때, 나의 반응은?",
    answers: ["우와! 홈 화면 진짜 깔끔하다. <br><br>색상도 멋지네!",
      "음... <br><br>카메라 렌즈는 좀 더 옆으로 조정하면 좋을 텐데.",
      "배터리 수명은? 아무런 정보도 없이 이걸 샀단 말이야?",
      "속도가 얼마나 빨라? 스펙 좀 보여줘봐. <br><br>내가 테스트 해볼게."],
    score: [1, 3, 2, 0]
  },

  { question: "새로운 보드게임을 시작하려고 하는데 규칙이 복잡하다. <br><br>이때, 나의 행동은?",
  ...
  { question: "식당에서 음식을 기다리는데 너무 오래 걸린다. <br><br>이때, 나의 행동은?",
  ...
  { question: "집에서 영화를 보다가 화면이 갑자기 꺼졌다. <br><br>이때, 나의 행동은?",
    answers: ["TV를 다시 켜보고 전원코드를 확인한다. <br><br>뭐가 문제일까?",
      "리모컨 배터리가 문제인가? 이참에 리모컨 버튼 배치를 바꿀 수 있을까?",
      "음... <br><br>기술적 문제? 이 상황에 대비한 리모콘이 필요했나?",
      "음... <br><br>갑자기 꺼진 화면이 뭔가 감성적이지 않나? 영화의 일부분일지도!"],
    score: [0, 1, 2, 3]
  },

  { question: "길을 걷다가 스마트폰 알림이 왔는데, 화면이 안 켜진다. <br><br>이때, 나의 행동은?",
    answers: ["아... <br><br>고장인가? 가끔은 이런 불완전함이 멋있어.",
      "화면이 안 켜지네? 스크립트가 멈춘건가?",
      "이럴 줄 알고 예비용 핸드폰을 가지고 다니지!",
      "전원을 껐다 켠다. <br><br>리셋은 모든 문제를 해결한다!"],
    score: [3, 1, 2, 0]
  },

  { question: "커피 머신이 고장 났다. <br><br>아침 커피를 못 마신다. <br><br>이때, 나의 행동은?",
    answers: ["커피 머신의 터치 패널이 반응이 없는 걸까? 인식 범위가 문제 있나?",
      "고장이 나도 이 커피 머신의 디자인은 여전히 멋져. <br><br>마치 작품 같아.",
      "커피를 마시지 못하면 능률이 떨어지는데..!!",
      "커피 머신을 열어 내부를 살펴본다. <br><br>이건 내가 고칠 수 있어!"],
    score: [1, 3, 2, 0]
  },

  { question: "옷을 사러 갔는데, 옷이 마음에 들지 않는다. <br><br>이때, 나의 행동은?",
    answers: ["이 옷은 나의 라이프 스타일에 맞지 않아. <br><br>내 옷장을 다시 계획해봐야겠어.",
      "옷의 패턴과 색상이 어울리지 않네. <br><br>뭔가 디자인을 바꾸고 싶다.",
      "옷의 품질을 분석하고, 원단과 재봉 상태를 점검한다.",
      "음... <br><br>옷은 마음에 들지 않지만, 옷걸이 디자인은 훌륭해!"],
    score: [2, 1, 0, 3] },
]);
...
<template>
  <div class="quiz-container">
    <div class="question-box">
      ...
      <div class="box">
        <div class="question">
          <p>{{ currentQuestionIndex + 1 }}. <br><br>{{ questions[currentQuestionIndex].question }}</p>
        </div>
